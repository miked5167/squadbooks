...
import { COUNTED_STATUSES } from '@/lib/constants/transaction-status'
...
export async function getAssociationFinancials(
  ...
  try {
    ...
    // Collect all team IDs to query transactions for category breakdown
    const teamInternalIds = associationTeams.map(at => at.team?.id).filter(Boolean) as string[]

    // Get all counted transactions for category breakdown
    // Use COUNTED_STATUSES to include both legacy (APPROVED) and current (VALIDATED, RESOLVED) statuses
    ...
    // Calculate totals from team snapshots (same source as team pages)
    for (const at of associationTeams) {
      ...
      // Get spent from snapshot (this matches what team pages show)
      if (snapshot?.spent) {
        totalSpent += decimalToNumber(snapshot.spent)
      }
    }
    ...
    const teams = associationTeams
      .map(at => {
        ...
        // Get spent from snapshot (same as team pages)
        const spent = snapshot?.spent ? decimalToNumber(snapshot.spent) : 0
