...
/**
 * Transaction statuses that count toward "spent" amount in financial calculations
 * Includes both legacy (APPROVED, APPROVED_AUTOMATIC) and current (VALIDATED, RESOLVED) statuses
 * to ensure backward compatibility and consistent calculations across the codebase
 */
export const COUNTED_STATUSES: TransactionStatus[] = [
  'APPROVED',
  'APPROVED_AUTOMATIC',
  'VALIDATED',
  'RESOLVED',
...
/**
 * Check if a transaction status counts toward spending
 */
export function isCountedStatus(status: TransactionStatus): boolean {
