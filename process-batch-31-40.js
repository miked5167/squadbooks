...
/**
 * Process batch 31-40 associations
 * Uses proven fixed-extraction logic
 */

const batch = JSON.parse(fs.readFileSync('next-batch-31-40.json', 'utf8'));
...
(async () => {
  console.log('='.repeat(80));
  console.log('BATCH PROCESSING - ASSOCIATIONS 31-40');
  console.log('='.repeat(80));
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    // Progressive save
    fs.writeFileSync('batch-31-40-results.json', JSON.stringify(results, null, 2));
  }
  ...
  // Summary
  console.log('\n\n' + '='.repeat(80));
  console.log('BATCH 31-40 SUMMARY');
  console.log('='.repeat(80));
  ...
  console.log('\n\nResults saved to: batch-31-40-results.json');

  // Show validated results
  console.log('\n' + '='.repeat(80));
  console.log('VALIDATED ASSOCIATIONS');
  console.log('='.repeat(80));
  ...
  // Cumulative summary
  console.log('\n' + '='.repeat(80));
  console.log('CUMULATIVE PROGRESS (BATCHES 1-40)');
  console.log('='.repeat(80));
  console.log('Total associations processed: 40');
  console.log('Estimated total successful: 19 (from batches 1-30) + ' + successCount + ' = ' + (19 + successCount));
})();
