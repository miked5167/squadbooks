...
const batch = JSON.parse(fs.readFileSync('new-batch-141-160.json', 'utf8'));
...
(async () => {
  console.log('='.repeat(80));
  console.log('NEW BATCH 141-160 (Excel File Associations)');
  console.log('='.repeat(80));
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    fs.writeFileSync('new-batch-141-160-results.json', JSON.stringify(results, null, 2));
  }

  await browser.close();

  console.log('\n' + '='.repeat(80));
  console.log('NEW BATCH 141-160 SUMMARY');
  console.log('='.repeat(80));
  console.log(`Processed: ${results.length} | Success: ${successCount} | Rate: ${((successCount / results.length) * 100).toFixed(1)}%`);
  console.log('Results saved to: new-batch-141-160-results.json');
})();
