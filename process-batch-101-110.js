...
const batch = JSON.parse(fs.readFileSync('next-batch-101-110.json', 'utf8'));
...
(async () => {
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    fs.writeFileSync('batch-101-110-results.json', JSON.stringify(results, null, 2));
  }

  await browser.close();

  console.log('\n' + '='.repeat(80));
  console.log('BATCH 101-110 SUMMARY');
  console.log('='.repeat(80));
  console.log(`Processed: ${results.length} | Success: ${successCount} | Rate: ${((successCount / results.length) * 100).toFixed(1)}%`);
  console.log('Results saved to: batch-101-110-results.json');
})();
