...
const batch = JSON.parse(fs.readFileSync('next-batch-131-140.json', 'utf8'));
...
(async () => {
  console.log('='.repeat(80));
  console.log('BATCH 131-136 (FINAL BATCH)');
  console.log('='.repeat(80));
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    fs.writeFileSync('batch-131-140-results.json', JSON.stringify(results, null, 2));
  }

  await browser.close();

  console.log('\n' + '='.repeat(80));
  console.log('BATCH 131-136 SUMMARY (FINAL BATCH)');
  console.log('='.repeat(80));
  console.log(`Processed: ${results.length} | Success: ${successCount} | Rate: ${((successCount / results.length) * 100).toFixed(1)}%`);
  console.log('Results saved to: batch-131-140-results.json');
})();
