...
must_haves:
  truths:
    - 'Multi-team queries use composite indexes (not sequential scans)'
    - 'Dashboard loads in under 2 seconds for 50 teams with 1000 transactions each'
    - 'Association user querying all teams executes single batch query (no N+1)'
    - 'Query execution plan shows Index Scan, not Seq Scan or Bitmap Heap Scan'
