...
<task type="auto">
  <name>Enhance TransactionsSection Component for Association Users</name>
  <files>app/association/[associationId]/teams/[teamId]/TransactionsSection.tsx</files>
  <action>
Enhance existing `TransactionsSection.tsx` component to support association user context with read-only enforcement.
...
1. **Update component props**:
   - Add `isAssociationUser?: boolean` prop (if not already present)
   - Existing `teamId: string` prop should remain
   - Component should accept these props for flexibility

2. **Transaction fetching**:
   - Fetch transactions using `/api/transactions?teamIds={teamId}&limit=50`
   - Pre-filter to single team (no TeamFilter component needed)
   - Implement cursor pagination for load more functionality

3. **Transaction table structure**:
   - Display table with columns: Date, Vendor, Category, Amount, Receipt Status
   - NO team column (already in team context - user knows which team)
   - Same table structure as association transactions page minus team column
   - Clickable rows to open transaction details drawer

4. **Update TransactionDetailsDrawer integration**:
   - Ensure component imports and renders `TransactionDetailsDrawer`
   - Pass `isReadOnly={isAssociationUser ?? false}` prop
   - When `isAssociationUser` is true, drawer should be read-only
   - When `isAssociationUser` is false (team users), drawer allows editing
...
5. **Loading and empty states**:
   ...
   - Show "View All Transactions" link to association transactions page (if association user)
...
```
  ...
  <done>
- TransactionsSection component enhanced for association user context
...
- TransactionDetailsDrawer opens with appropriate read-only mode based on user type
