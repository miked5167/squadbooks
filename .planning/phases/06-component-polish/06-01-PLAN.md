...
must_haves:
  ...
  artifacts:
    ...
    - path: app/association/page.tsx
      provides: Health status badges with accessible icon patterns
      contains: 'aria-hidden="true"'
  key_links:
    ...
    - from: ValidationComplianceCard and association/page.tsx
      to: Lucide icons
...
<task type="auto">
  <name>Task 4: Add accessible status badges to association overview page</name>
  <files>app/association/page.tsx</files>
  <action>
Enhance health status and severity badges with accessibility features:

**Lines 395-418: Update HealthBadge component:**

The HealthBadge currently uses custom className strings. Update to use Badge variants with icons:

First, add icon imports at top of file (after existing lucide-react imports on line 18):
...
Then update HealthBadge function (lines 395-418):
...
**Lines 420-433: Update SeverityBadge component:**

Replace custom className logic with consistent Badge variant usage:
...
function SeverityBadge({ severity }: { severity: string }) {
  const config: Record<string, { variant: 'success' | 'warning' | 'destructive'; icon: any }> = {
    ...
    info: { variant: 'success', icon: CheckCircle2 },
  }

  const item = config[severity.toLowerCase()] || config.warning
  const Icon = item.icon

  return (
    <Badge variant={item.variant} className="flex items-center gap-1.5">
      <Icon className="h-3.5 w-3.5" aria-hidden="true" />
...
- Use Badge component variants instead of custom className strings (DRY principle)
- Add icons for redundant visual signaling (color-blind accessibility)
- aria-hidden on icons (decorative, meaning in text)
- sr-only spans provide screen reader context
- Consistent with other status badges across dashboard
- Icon size h-3.5 (14px) matches other status indicators
  </action>
  <verify>
  Run: `npm run build` to confirm TypeScript compilation
  Check: HealthBadge and SeverityBadge use Badge component variants
  Check: Icons have aria-hidden="true"
  Check: sr-only spans present with context text
  Visual check: Badges render with icons next to text
  </verify>
  <done>
  Health and severity badges use consistent Badge variants with icons
  Screen readers announce "Team health: Healthy" and "Alert severity: Critical"
  Icons provide redundant visual signaling for color-blind users
  WCAG 2.1 AA accessible pattern implemented across association overview
  </done>
  </task>
...
1. **Screen Reader Test:**
   ...
   - Verify badges announce meaningful context: "Budget status: On Track", "Team health: Healthy"
...
4. **Visual Regression Check:**
   - Visit all dashboard pages: /association/[id]/overview, parent dashboard
