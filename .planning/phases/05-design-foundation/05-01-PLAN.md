...
must_haves:
  truths:
    ...
    - 'CardTitle text-lg default works correctly across all app pages (dashboard, transactions, teams, settings)'
  artifacts:
  ...
  key_links:
    ...
    - from: 'components/ui/card.tsx CardTitle'
      to: 'computed 18px font size'
      via: 'text-lg class inheritance'
      pattern: 'text-lg'
---
...
**Scope Note:** This plan modifies `components/ui/card.tsx`, a base component used across 107 files application-wide. CardHeader space-y change (minimal risk) and CardTitle text-lg default (beneficial app-wide) will affect all pages using Card. Cross-app verification required.
</objective>
...
**IMPORTANT - Cross-app impact:** This changes a base component used in 107 files across the app. The text-lg default will affect ALL CardTitle usages app-wide (transactions page, teams page, settings, budget pages, etc.). This is INTENTIONAL and BENEFICIAL - 18px card headings improve hierarchy everywhere. Checkpoint verification will test key pages.
...
Verify text-lg is actually applied:

```bash
# Check that text-lg appears in CardTitle className definition
grep -A 2 "const CardTitle" components/ui/card.tsx | grep "text-lg"
```

</verify>
<done>
CardHeader uses space-y-2 (8px), CardTitle includes text-lg as default base class. Card component establishes systematic foundation for all cards across the application (dashboard, transactions, teams, settings).
</done>
</task>
...
<done>
Dashboard page and card components use systematic typography hierarchy. Changes applied:

**Checklist:**

- [ ] app/dashboard/page.tsx: 2 CardTitle instances have text-base removed (Exceptions/Compliance cards)
- [ ] components/dashboard/BudgetCategoryList.tsx: 2 CardTitle instances have text-base removed (lines 25, 54)
- [ ] components/dashboard/QuickActionsCard.tsx: 1 CardTitle instance has text-base removed (line 14)
- [ ] Labels retain explicit text-sm classes (no changes to label sizing)
- [ ] Badges retain text-xs via Badge component (no changes)
- [ ] Metrics retain explicit text-3xl classes (no changes)

**Result:** CardTitle defaults to 18px (text-lg), labels consistently use 14px (text-sm), badges use 12px (text-xs), metrics use 30px (text-3xl). Typography creates clear visual hierarchy matching QuickBooks design patterns.
</done>
</task>
...
**Cross-app impact:** CardTitle text-lg default now applies to ALL pages using Card (transactions, teams, settings, budget pages, etc.). This is intentional for consistent hierarchy app-wide.
</what-built>
<how-to-verify>
...
4. **Verify text-lg inheritance is working:**

   Open browser DevTools (F12), inspect any CardTitle element:

   ```bash
   # Inspect computed font-size on CardTitle elements
   # Should show: font-size: 18px (from text-lg class)
   # NOT: 16px (text-base) or 14px (text-sm)
   ```

   Check 2-3 CardTitle elements to confirm text-lg is applied.

5. **Verify visual hierarchy (scannable test):**
...
6. **Verify spacing (4px grid alignment):**
...
8. **CROSS-APP VERIFICATION (critical):**

   Test CardTitle text-lg on other key pages:

   **a) Transactions page:**
   - Visit http://localhost:3000/transactions
   - Check transaction list card title (should be 18px)
   - Verify no layout breaks, text overflow

   **b) Teams page (if accessible):**
   - Visit association teams page
   - Check team cards titles (should be 18px)
   - Verify cards look professional, not broken

   **c) Settings pages:**
   - Visit http://localhost:3000/settings/team or /settings/bank-accounts
   - Check any card titles (should be 18px)
   - Verify no visual regressions

9. **Compare to v1.0 baseline:**
...
- ✓ DevTools confirms 18px computed font-size on CardTitle elements
- ✓ Transactions/teams/settings pages show no regressions from text-lg default
...
- ✗ DevTools shows font-size other than 18px on CardTitle
- ✗ Other pages (transactions, teams) have layout breaks or visual issues
  </how-to-verify>
  <resume-signal>
  Type "approved" if typography hierarchy is clear, spacing feels professional, and cross-app pages (transactions, teams, settings) show no regressions.

If issues found, describe:

- Which component/page has the issue
- What's wrong (too large/small, cramped, overflowing, layout break, etc.)
...
```bash
# Verify CardTitle has text-lg in its className definition
grep -A 2 "const CardTitle" components/ui/card.tsx | grep "text-lg"
```
...
- DevTools confirms 18px computed font-size on CardTitle elements
- Cross-app pages (transactions, teams, settings) show no regressions
  </verification>
...
4. **No regressions (dashboard AND cross-app):**
   - Dashboard loads without errors
   - Transactions page loads without errors, CardTitle works correctly
   - Teams/settings pages load without errors, CardTitle works correctly
   - No layout breaks or alignment issues anywhere
...
6. **Human verification approved:**
   ...
   - Spacing feels professional and consistent
   - No visual issues or layout problems reported on dashboard
   - Transactions/teams/settings pages verified with no regressions
     </success_criteria>
...
- Cross-app impact assessment (which pages tested, results)
